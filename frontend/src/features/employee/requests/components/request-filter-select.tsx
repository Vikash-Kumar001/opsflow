...
type RequestFilterOption<TValue extends string> = {
  value: TValue;
  label: string;
};

type RequestFilterSelectProps<TValue extends string> = {
  label: string;
  value: TValue | "";
  options: Array<RequestFilterOption<TValue>>;
  onChange: (value: TValue | undefined) => void;
  className?: string;
};

export function RequestFilterSelect<TValue extends string>({
  label,
  value,
  options,
  onChange,
  className,
}: RequestFilterSelectProps<TValue>) {
  return (
